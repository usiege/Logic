...
{ 
    ...
    if (!nonpointer) {
        ...
        // isa.nonpointer is part of ISA_MAGIC_VALUE
        newisa.has_cxx_dtor = hasCxxDtor;
        newisa.shiftcls = (uintptr_t)cls >> 3; // 字符串  0101010 (数字 - 理解)
#endif
...
objc_object::rootRetain(bool tryRetain, bool handleOverflow)
{
    ...
    do {
        ...
        if (slowpath(tryRetain && newisa.deallocating)) { // dealloc 源码
            ClearExclusive(&isa.bits);
            if (!tryRetain && sideTableLocked) sidetable_unlock();
            return nil;
        }
        ...
        if (slowpath(carry)) { // 操作散列表 extra_rc 满/2  -> 开锁散列表
...
objc_object::rootRetainCount()
{
    ...
    if (bits.nonpointer) {  // 读 没有写
        uintptr_t rc = 1 + bits.extra_rc; // alloc 0 + 1 = 1
